...
workers.validateCheckData = (originalCheckData) => {
    ...
    if (originalCheckData.id &&
        ...
        originalCheckData.timeoutSeconds) {
            ...
            console.log("Error: One of the checks is not properly formatted it. Skipping it");
        }
};
...
workers.performCheck = (originalCheckData) => {
    ...
    const path = parsedUrl.path; // Using path and not pathname, because we want full querystring
...
// Special logic for acomodating a check that has never been tested before (don't alert on that one)
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            // Send the new check data to the next phase of the project if needed
            if (alertWarranted) {
...
workers.alertUserToStatusChange = (newCheckData) => {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            console.log("Success: User was alerted to a status change in thier check, via SMS", msg);
        } else {
            ...
            logs.forEach((logName) => {
                ...
                _logs.compress(logId, newFileId, (err) => {
                    if (!err) {
                        ...
                        console.log("Error compression one of the log files", err);
                    }
                });
            });
        } else {
