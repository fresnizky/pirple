  ...
  if (data.method == 'get') {
    ...
    helpers.getTemplate('index', templateData, (err, str) => {
      if (!err && str) {
        ...
        helpers.addUniversalTemplates(str, templateData, (err, finalString) => {
          if (!err && str) {
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {'Error': 'Errors encountered while attempting to delete all of the users checks. All the checks may not have been deleted from the system successfully'});
                        }
                      }
                    });
                  });
  ...
  if (id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
        ...
        // Verify that the given token is valid for belongs to the user who created the check
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      ...
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
          ...
          handlers._tokens.verifyToken(token, checkData.userPhone, (tokenIsValid) => {
            if (tokenIsValid) {
              // Update the checkData object where necessary
              if (protocol) {
              ...
              if (!err) {
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    // Removed the deleted check from the list of checks
